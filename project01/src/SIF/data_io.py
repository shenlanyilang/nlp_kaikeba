...
def getWordmap(textfile):
    words={}
    We = []
    f = codecs.open(textfile,'r',encoding='utf8')
    lines = f.readlines()
...
def sentences2idx2(sentences, words, weight4ind):
    '''
    ...
    for i,sent in enumerate(sentences):
        terms = sent.split()
        terms_indices = [words[term] for term in terms if term in words]
        if len(terms_indices) == 0:
            sents_no_ind.append(i)
            continue
        sents_indices.append(terms_indices)
        sents_words_weights.append([weight4ind[indice] for indice in terms_indices])
    return sents_indices,sents_words_weights,sents_no_ind
...
def getWordWeight(weightfile, a=1e-3):
    ...
    word2weight = {}
    with codecs.open(weightfile,'r',encoding='utf8') as f:
        lines = f.readlines()
    N = 0
    for i in lines:
        i=i.strip()
